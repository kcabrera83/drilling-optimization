...
def test_models():
    results = {}
    results['rop'] = evaluate_model(os.path.join(MODEL_DIR, 'rop_predictor.pkl'), X, y)
    results['torque'] = evaluate_model(os.path.join(MODEL_DIR, 'torque_predictor.pkl'), X, y)
    results['vibration_analyzer'] = evaluate_model(os.path.join(MODEL_DIR, 'vibration_analyzer.pkl'), X, y)
    return results
